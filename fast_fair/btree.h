...
class btree{
  ...
  public:
    ...
    void btree_search_range(entry_key_t, entry_key_t ,long); 
    void printAll();
  ...
  //   printf("get lock\n");
	if(with_lock)
	{
	 hdr.mtx->lock();
      ...
      //bool ret=remove_rebalancing(bt,key);
      hdr.mtx->unlock();
	return ret;
	}
	bool ret = remove_key(key);
...
//printf("13 \n");
  ...
  //      printf("131 \n");
    ...
    void linear_search_range
      (entry_key_t min, entry_key_t max, unsigned long* buf, long len) {
        int i=0;
        int off = 0;
        uint8_t previous_switch_counter;
        page *current = this;
...
void openPmemobjPool(char* pathname) {
  ...
  if (file_exists(pathname) != 0) {
    ...
    if ((pop = pmemobj_create(pathname, POBJ_LAYOUT_NAME(btree),
                              (uint64_t)200ULL * 1024ULL * 1024ULL * 1024ULL, 0666)) == NULL) {
...
(entry_key_t min, entry_key_t max, long len) {
  page *p = (page *)root;
  unsigned long buf[120]={0};
  while(p) {
    if(p->hdr.leftmost_ptr != NULL) {
      ...
      // Found a leaf
      p->linear_search_range(min, max, buf, len);
