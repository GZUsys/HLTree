...
#define LEAF_KEY_NUM    (28) 

#define  NON_LEAF_ENTRYS 31
...
public:
    ...
    bool isFull(void) { return (head.bitmap == 0xfffffff);}
...
restart:
    ...
    uint32_t fbitmap = lp->head.bitmap;
    ret_pos = -1;
    while (fbitmap) {
...
restart:
  ...
  //printf("min max key is %lld, %lld, %lld, %ld\n",lp->Min_Key,lp->Max_Key, lp->num(),lp->head.hangin);
    while (lp->next != NULL && key > lp->getmaxkey())
    {
        ...
        {
	    //if(tempt->head.hangin==0) printf("error node is occur\n");
            pw++;
            lp = tempt;
        }
        else
            ...
            if ((key > lp->getdistance()) && (tree_meta->root_level > 0))
            {
                if (lp->next !=NULL && lp->next->head.hangin)
                {
                    //node is hang in leaf-node layer
                        ...
                        uint32_t bitmap = lp->head.bitmap;
    ...
    else
    {
        ...
        uint32_t freed_slots = lp->head.bitmap;
	...
	lp->Max_Key = lp->k(sorted_pos[split-1]);
        ...
        if (key < split_key)
        {
            uint32_t lbitmap = lp->head.bitmap;
...
restart:
    ...
    while (lp != NULL)
    {
        ...
        uint32_t node_bitmap = lp->head.bitmap;
        node_bitmap &= 0xfffffff;
        if (order)
        {
            int scount = 1;
            long long slot[32] = { 0 };
