...
uint64_t latency[test_thread]={0}, insert_nb[test_thread] = {0},insert_nbs = 0;
...
int main(int argc, char **argv)
{
    struct option long_options[] = {
        ...
        {"operate",                   required_argument, NULL, 'd'},
   ...
   assert(nb_threads > 0);
   printf("Nb threads   : %d,%d\n",  nb_threads, operation); 
    ...
    btree *bt;
    bt = new btree(NULL);
    bt->set_first_leaf((uint64_t)buffer[0],(char*)buffer[0]);
    initial=buffer.size();
 ...
 /*
    for(int che =0; che<50000000;che++)
    {
	uint64_t kks = buffer[che];
	int poss;
	uint64_t res=(uint64_t)bt->lookup(kks, &poss);
	//if(res!=kks)printf("error\n");
    }
    bt->print();
    */
