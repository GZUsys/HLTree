...
void pmem_drain(void) { _mm_sfence();/*sfencenum++;*/ }
...
static void split_leaf_node(ptst_t *ptst, fptree_t *tree, fptree_node_t *node,
     ...
     * lookups.
     */
    //printf("begin create a leaf node\n");
    nleaf = leaf_create(ptst);
    //printf("create a leaf node\n");
...
/*query the entries by the satrt key and length*/

bool fptree_scan(fptree_t *tree, setkey_t key, int len) {
    fptree_node_t *root = tree->root;
    unsigned idx;
    fptree_leaf_t *leaf;
    uint32_t v;
...
forward:
    ...
    while(true)
    {
    setkey_t temp[NODE_MAX]={0};
    int length=0;
    unsigned i=0;
    unsigned nkeys = leaf->nkeys;
    NOSMP_ASSERT(validate_leaf(leaf));
    if((newlen+nkeys) < len && newlen>0)
    {
	    length=newlen;
	 for (i = 0; i < NODE_MAX; i++)
    	{
        if (check_bitmap(leaf->bitmap, i) && key < leaf->slot_kv[i].key) {
            results[length]=leaf->slot_kv[i].key;
            atomic_thread_fence(memory_order_acquire);
            length++;
        }
    	}
    ...
    {
        if (check_bitmap(leaf->bitmap, i) && key < leaf->slot_kv[i].key) {
            temp[length]=leaf->slot_kv[i].key;
	    atomic_thread_fence(memory_order_acquire);
	    length++;
        }
    }
    ...
    for(int k=0; k < length && newlen < len;k++)
    {
	results[newlen]=temp[k];
	newlen++;
    }
    }
    if(leaf->next!=NULL && newlen < len)
    {
	    leaf = leaf->next;
    }
    else{break;}
    }
    //printf("the len is %lld %lld\n",len,newlen);
    //critical_exit(ptst);
    return found;
    // if (found) return lv;
    // return NULL;
