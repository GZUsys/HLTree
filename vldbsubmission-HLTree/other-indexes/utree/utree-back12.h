...
//long clwb_num[10]={0};
//long sfence_num[10]={0};
//thread_local int work_id;
...
inline void mfence()
{
  asm volatile("mfence":::"memory");//sfence_num[work_id]++;
}
    ...
    bool remove_rebalancing(btree* bt, entry_key_t key, bool only_rebalance = false, bool with_lock = true) {
        ...
        if (total_num_entries > cardinality - 1) { // Redistribution
            ...
            if (num_entries < left_num_entries) { // left -> right
                ...
                new_sibling->hdr.pred_ptr = hdr.pred_ptr;
                ...
                if (hdr.leftmost_ptr == nullptr) {
                    ...
                    if(new_sibling->hdr.sibling_ptr != NULL) {
			new_sibling->hdr.sibling_ptr->hdr.mtx->lock();
                        new_sibling->hdr.sibling_ptr->hdr.pred_ptr = new_sibling;
			new_sibling->hdr.sibling_ptr->hdr.mtx->unlock();
			}
           ...
           // clflush((char*)&(hdr.is_deleted), sizeof(uint8_t));
            ...
            if(hdr.sibling_ptr != NULL) {
                hdr.sibling_ptr->hdr.mtx->lock();
                hdr.sibling_ptr->hdr.pred_ptr = hdr.pred_ptr;
                hdr.sibling_ptr->hdr.mtx->unlock();
            }
...
void openPmemobjPool() {
  printf("use pmdk!\n");
  char pathname[100] = "/mnt/ext4/utree/pool";
...
/*
        if(p==p->hdr.sibling_ptr->hdr.pred_ptr)
        {
           printf("pre is true\n");
        }
*/
...
retry:
  ...
  while(prev != NULL && prev->next->key<cur->key)
  {
     prev=prev->next;
  }
  if (prev->next != cur) { 
...
void btree::btree_delete(entry_key_t key) {
  ...
  if(p) {
    if(!p->remove_rebalancing(this, key)) {
      //btree_delete(key);
    }
  }
  else {
